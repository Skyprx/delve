...
func (p *gdbProcess) Connect(conn net.Conn, path string, pid int, debugInfoDirs []string, stopReason proc.StopReason) (*proc.Target, error) {
	...
	// It may seem the case, that actually a register is used on arm64
	// instead of the TLS. See https://github.com/golang/go/issues/24793
...
func (regs *gdbRegisters) Get(n int) (uint64, error) {
	...
	fmt.Printf("Register %d not in switch\n", n)
...
// SetDW will set the value of the DX register to the given value.
